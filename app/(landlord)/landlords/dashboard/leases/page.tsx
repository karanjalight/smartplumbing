import { CheckCircle2, Clock, FileSignature, Rocket, ScrollText } from "lucide-react";
...
export const metadata = {
  ...
  description: "Track tenancy agreements across your portfolio.",
};
...
export default async function LandlordLeasesPage() {
  ...
  const landlord = await fetchSignedInLandlord(client);
  if (!landlord) {
    redirect("/auth/login");
  }
  // RLS (leases_landlord_full) already scopes this to the landlord's leases.
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Tenancy agreements across your portfolio. Create new leases from the
            onboarding flow.
          </p>
        </div>
        <Button
          ...
          render={<Link href="/landlords/dashboard/onboarding" />}
        >
        ...
        {leases.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Add a tenant to a unit in the onboarding flow, then create their lease.
              </p>
            </div>
            <Button
              variant="outline"
                ...
                {leases.map((l) => (
                  ...
                  >
                    <td className="px-5 py-3.5">
                      <Link
                        href={`/landlords/dashboard/leases/${l.id}`}
